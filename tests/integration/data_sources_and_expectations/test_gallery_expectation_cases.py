"""Unit coverage for the gallery-tier case record and its shared fixture data.
...
def test_shared_frame_carries_no_extra_tables() -> None:
    ...
    # assert the positive property instead -- GALLERY_FIXTURE_DATA's column set is *exactly* the
    ...
    assert isinstance(GALLERY_FIXTURE_DATA, pd.DataFrame)
    assert set(GALLERY_FIXTURE_DATA.columns) == expected_columns
    assert set(GALLERY_FIXTURE_DATA.columns).isdisjoint(set(GALLERY_EXTRA_TABLE_DATA.columns))
...
def test_module_imports_with_no_data_source_dependency_installed() -> None:
    ...
    ``gallery_expectation_case_table.py``, which imports ``great_expectations`` to build those
    ...
    probe = textwrap.dedent(
        ...
        assert len(GALLERY_FIXTURE_DATA) > 0
        assert len(GALLERY_EXTRA_TABLE_DATA) > 0
        ...
        assert not registry_modules, (
            f"gallery_expectation_cases.py pulled in the data_source_config package: "
        ...
        assert not sqlalchemy_modules, (
            f"gallery_expectation_cases.py pulled in sqlalchemy: {sorted(sqlalchemy_modules)}"
        )
        ...
        assert not great_expectations_modules, (
            "gallery_expectation_cases.py pulled in great_expectations itself (which "
